  ...
  .view(({ routePath }) => (
    <>
      <NextHead>
        ...
        {/* Preload the fonts */}
        {/* {DEFAULT_FONTS_TO_PRELOAD.map(({ name: fontName, weights: fontWeghts }) =>
        fontWeghts.map((fontWeight) => (
          <link
            rel="preload"
            href={`/static/fonts/${fontName.toLocaleLowerCase()}/${fontName.toLocaleLowerCase()}-${fontWeight}.woff2`}
            as="font"
            type="font/woff2"
            crossOrigin=""
          />
